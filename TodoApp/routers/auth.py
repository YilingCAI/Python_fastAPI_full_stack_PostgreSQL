...
SECRET_KEY = '75cc942eb455e565c15d1e686996bd24ca385da081d25d8e6f2253ac2f451b91'
ALGORITHM = 'HS256'

bcrpt_context = CryptContext(schemes=['bcrypt'], deprecated='auto')
oath2_bear = OAuth2PasswordBearer(tokenUrl='auth/token')
...
async def get_current_user(token: Annotated[str, Depends(oath2_bear)]):
    try:
        ...
        if username is None or user_id is None:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Could not validate user, jwt error.')
